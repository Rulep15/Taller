    ...
    <div class="col-md-8 col-md-offset-0" style=" margin-top: -14px; border: none;">
        ...
        <div class="form-group">
            ...
            <div class="col-lg-6 col-sm-6 col-xs-6">
                <input class="form-control" type="date" id="iddesde" name="vdesde" value="<?= date('Y-m-d'); ?>" max="<?= date('Y-m-d'); ?>" min="" required="">
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-lg-2 col-sm-2 col-xs-2">Hasta</label>
            <div class="col-lg-6 col-sm-6 col-xs-6">
                <input class="form-control" type="date" id="idhasta" name="vhasta" value="<?= date('Y-m-d'); ?>" max="<?= date('Y-m-d'); ?>" min="" disabled="" required="">
            </div>
        </div>
...
<script>
    function enviar() {
        window.open("/Taller/informes/compras/presupuesto/presupuesto_print.php?vdesde=" + "'" + $('#iddesde').val() + "'" + "&vhasta=" + "'" + $('#idhasta').val() + "'" + "&vopcion=" + $('#opcion').val(), '_blank');
    }
</script>
<script>
    //     function hasta() {
